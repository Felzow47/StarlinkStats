...
class CompactDropZone:
    """Zone oÃ¹ dÃ©poser une carte compacte (insertion aprÃ¨s anchor_key)."""
...
@dataclass(frozen=True)
class GridSnapSlot:
    """Emplacement grille : insertion ou remplacement (Ã©change) d'une carte."""
...
def metrics_area_width(wrap: QWidget | None = None) -> int:
    """Largeur utile de la grille (fixe, indÃ©pendante d'un conteneur Ã©largi)."""
    _ = wrap
...
def _resolve_columns(
    ...
    """Colonne 0/1 par carte â€” colonne fixe conservÃ©e au dÃ©placement."""
...
def preview_normalize_spans(
    ...
    """Comme normalize_metric_spans, sans persistance (aperÃ§u drag)."""
...
def compute_grid_geometries(
    ...
    *,
    ...
    """Positions absolues â€” chaque colonne a sa propre hauteur cumulÃ©e."""
...
def compute_compact_drop_zones(
    ...
    *,
    ...
    """Emplacements = position rÃ©elle de la carte aprÃ¨s insert_after(anchor)."""
...
def compute_grid_snap_slots(
    ...
    *,
    ...
    """Emplacements valides : Ã©change sur chaque carte + insertions dans l'ordre."""
    if not tiles or not drag_key:
        return []
...
def nearest_grid_snap_slot(
    ...
    *,
    ...
    """Slot le plus proche dans la colonne visÃ©e par le curseur."""
    if not slots:
        return None
...
def detect_geometry_overlaps(geos: dict[str, QRect]) -> list[dict]:
    """DÃ©tecte les rectangles qui se chevauchent (debug layout)."""
    keys = list(geos.keys())
    overlaps: list[dict] = []
