    ...
    *,
    ...
    def _add(
        ...
        *,
        ...
        seen.add(sig)
        targets.append(
            DropTarget(QRect(geo), action, anchor_key, grid_col, stack=stack)
        )
    ...
    for anchor_key in base:
        ...
        if drag_compact == anchor_compact and anchor_geo is not None:
            if anchor_geo.width() <= col_w + spacing // 2:
                col = _anchor_col(
                    anchor_key, anchor_prefs, static_geos, container_width, spacing
                )
                _add(QRect(anchor_geo), "swap", anchor_key, col)
    ...
    try:
        payload = {
            ...
            "runId": "grid-drag-v2",
            "hypothesisId": "N",
            "location": "grid_drag:compute_drop_targets",
            "message": "drop targets",
            "data": {
                "drag_key": drag_key,
    ...
    *,
    ...
    try:
        payload = {
            ...
            "runId": "grid-drag-v2",
            "hypothesisId": "P",
            "location": "grid_drag:pick_drop_target",
            "message": "picked",
            "data": {
                "pointer_col": pointer_col,
                "drag_compact": drag_compact,
                "action": result.action,
                "anchor": result.anchor_key,
                "col": result.grid_col,
                "rect_y": result.rect.y(),
            },
            "timestamp": int(time.time() * 1000),
        }
        with _LOG_PATH.open("a", encoding="utf-8") as f:
...
def apply_drop_target(
    ...
    *,
    ...
    if target.action == "insert_before":
        ...
        if drag_compact and not anchor_compact:
            insert_field_before(drag_key, target.anchor_key)
            set_card_grid_col(drag_key, target.grid_col)
            clear_stack_under(drag_key)
        elif drag_compact and anchor_compact:
            swap_field_order(drag_key, target.anchor_key)
            swap_card_grid_cols(drag_key, target.anchor_key)
        else:
            swap_field_order(drag_key, target.anchor_key)
            set_card_grid_col(drag_key, target.grid_col)
            clear_stack_under(drag_key)
